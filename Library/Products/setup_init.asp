...
origemRecep = request.QueryString("planoPaises")
familiar = request.QueryString("familiar")
nPax = cint(idadeMenor) + cint(idadeMaior)
clienteId = request.cookies("wlabel")("revId")
ip = request.ServerVariables("REMOTE_ADDR")
fone = request.QueryString("fone")
email = request.QueryString("email")
nomeCli = request.QueryString("nomeCli")
...
vSQLUpd = vSQLUpd + "   nome 				= '"&nomeCli&"' ;"+chr(0)
...
vSQLUpd = vSQLUpd + "   origemRecep         = '"&origemRecep&"' ;"+chr(0)
...
While not cotacaoRS.EOF	
	...
	Call CalcPrice(cotacaoRS("id"), dias, destino_id, vet_idadePax, familiar)		
